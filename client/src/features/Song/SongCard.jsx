...
const SongCard = ({ song }) => {
  ...
  return (
    <article className="group glass-effect rounded-xl shadow-lg p-2 md:p-4 transition-all duration-300 hover:scale-105 hover:neon-glow neon-border">
      <div className="relative bg-secondary-200/50 rounded-lg overflow-hidden">
        {song.coverImage ? (
          ...
          <FaCompactDisc className="w-full p-4 h-24 sm:h-40 text-neon animate-pulse-slow" />
        )}
        <Link
          to={`/songs/${song._id}`}
        ...
        <button
          onClick={handlePlay}
          className={`bg-${selectedTheme} absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 top-1/2 bg-opacity-50 hover:bg-opacity-100 text-white flex justify-center items-center w-14 h-14 active:bg-${selectedTheme}-50 active:bg-opacity-40 rounded-full transition-all duration-300 neon-glow hover:scale-110`}
        >
