...
const ArtisteCard = ({ artiste }) => {
  ...
  return (
    <article className="group w-24 md:w-32 lg:w-40 transition-all duration-300 hover:scale-105">
              <div className="relative glass-effect rounded-full w-24 md:w-32 lg:w-40 h-24 md:h-32 lg:h-40 shadow-lg neon-border hover:neon-glow">
        <Link to={`/artistes/${artiste._id}`}>
          {artiste.image ? (
            ...
            <FaUserAstronaut className="w-full pt-4 h-24 md:h-32 lg:h-40 rounded-full text-neon animate-pulse-slow" />
          )}
        </Link>
      </div>
