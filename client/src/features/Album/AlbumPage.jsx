  ...
  return (
    <section className=" text-gray-200">
      <Helmet prioritizeSeoTags>
        <title>{`${album.artiste.name} - ${album.title} MP3 Stream on RhythMix`}</title>
        <link
          rel="canonical"
          href={`https://RhythMix.vercel.app/albums/${album._id}`}
        />
        <meta
          name="description"
          content={`Stream ${album.title} by ${
            album.artiste.name
          } on RhythMix and enjoy other amazing music collections. Released ${formatDate(
            album.releaseDate
          )}`}
        />
        <meta
          property="og:title"
          content={`${album.artiste.name} - ${album.title} MP3 Stream on RhythMix`}
        />
        <meta
        ...
        <meta
          property="og:url"
          content={`https://RhythMix-server.vercel.app/albums/${album._id}`}
        />
